 ...
 * `'village/village-exterior'` — that indexes into public/img/manifest.json
 * (generated by scripts/photos.mjs, see docs/04-photography.md §4). The
 * manifest carries the natural width/height, the LQIP base64 placeholder,
 * and which responsive widths exist for each format, so a component can
 * build a full srcset (AVIF + WebP + JPEG fallback) from this one string
 * via src/content/manifest.ts's `resolveImage()` — no per-image width/height/
 * format bookkeeping needs to live in content files.
 */
export interface ImageRef {
  base: string; // manifest key: '<site>/<name>' — see public/img/manifest.json
